...
checkboxes and tree lines, on the right the resolved target per repo. Targets can
be set per repo **or at the group level** (inherited by the substructure).
...
▸ │  ├─ [x] deployment   → example-org/tools/deployment        ✓ transferred
 ...
 Options (failsafe):  1 [x] Issues/MRs/Labels/Milestones   2 [x] CI vars
                      3 [x] Settings   4 [x] URL rewrite
```
...
still counts as successfully migrated:
...
- **Container registry** — copies all images + tags registry-to-registry via
  [`skopeo`](https://github.com/containers/skopeo) (off by default; needs
  `skopeo` on PATH and tokens with registry access). Skipped with a warning if
  `skopeo` is missing or the registry is disabled.
...
- For the optional **container registry** copy: [`skopeo`](https://github.com/containers/skopeo)
  on PATH, and tokens with registry access (the `api` scope covers this).
...
| `a` / `N` | Select all / none |
| `1`–`6` | Toggle options (Issues/MRs, CI vars, Settings, URL rewrite, Releases, Container registry) |
...
- Container registry copy is registry-to-registry via `skopeo`; it can move a
  lot of data and needs `skopeo` installed. Package and model registries are
  **not** copied — they are usually rebuilt/republished by CI after migration.
